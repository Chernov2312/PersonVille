...
class HelperFunctionTests(TestCase):
    def test_normalize_house_text_with_dict(self):
        from city.views import _normalize_house_text

        test_dict = {'text': 'Тестовый текст', 'other': 'data'}
        result = _normalize_house_text(test_dict)
        self.assertEqual(result, 'Тестовый текст')

    def test_normalize_house_text_with_string(self):
        from city.views import _normalize_house_text

        result = _normalize_house_text('Простая строка')
        self.assertEqual(result, 'Простая строка')

    def test_normalize_house_text_with_none(self):
        from city.views import _normalize_house_text

        result = _normalize_house_text(None)
        self.assertEqual(result, None)

    def test_find_street_returns_correct_street(self):
        from city.views import _find_street

        city_result = {
            'streets': [
                {'trait': 'openness', 'name': 'Openness'},
                {'trait': 'conscientiousness', 'name': 'Conscientiousness'},
            ],
        }

        street = _find_street(city_result, 'openness')
        self.assertIsNotNone(street)
        self.assertEqual(street['trait'], 'openness')

    def test_find_street_returns_none_if_not_found(self):
        from city.views import _find_street

        city_result = {'streets': [{'trait': 'openness', 'name': 'Openness'}]}

        street = _find_street(city_result, 'invalid')
        self.assertIsNone(street)

    def test_find_house_returns_correct_house(self):
        from city.views import _find_house

        street = {
            'houses': [
                {'house_id': 'house_1', 'text': 'House 1'},
                {'house_id': 'house_2', 'text': 'House 2'},
            ],
        }

        house = _find_house(street, 'house_2')
        self.assertIsNotNone(house)
        self.assertEqual(house['house_id'], 'house_2')

    def test_find_house_returns_none_if_not_found(self):
        from city.views import _find_house

        street = {'houses': [{'house_id': 'house_1', 'text': 'House 1'}]}

        house = _find_house(street, 'invalid')
        self.assertIsNone(house)

    def test_refresh_street_progress_normalizes_houses(self):
        from city.views import _refresh_street_progress
        ...
        result = _refresh_street_progress(street)

        self.assertEqual(len(result['houses']), 2)
        self.assertTrue(all('house_id' in h for h in result['houses']))
        self.assertTrue(all('completed' in h for h in result['houses']))
        self.assertEqual(result['answered_count'], 0)
        self.assertFalse(result['completed'])

    def test_refresh_street_progress_counts_completed_houses(self):
        from city.views import _refresh_street_progress

        street = {
            'trait': 'openness',
            'houses': [
                {'house_id': 'h1', 'completed': True, 'base_text': 'Text'},
                {'house_id': 'h2', 'completed': False, 'base_text': 'Text'},
                {'house_id': 'h3', 'completed': True, 'base_text': 'Text'},
            ],
        }
    ...
    def test_refresh_city_progress_updates_all_streets(self):
        from city.views import _refresh_city_progress
        ...
        result = _refresh_city_progress(city_result)

        self.assertTrue(result['all_completed'])
        self.assertIn('is_finalized', result)
