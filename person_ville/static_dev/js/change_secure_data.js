...
function startButtonTimer(button, endDate) {
    if (!button || !endDate) {
        return;
    }

    const timerLabel = button.dataset.timerLabel || "Повторно через";
    const baseLabel = button.dataset.baseLabel || "Получить код снова";
    ...
    function tick() {
        ...
        button.textContent = `${timerLabel} ${formatCountdown(diff)}`;
...
function startNoteTimer(note, endDate, doneText) {
    if (!note || !endDate) {
        return;
    }
    ...
    function tick() {
        ...
        note.textContent = `${doneText} ${formatCountdown(diff)}`;
        window.setTimeout(tick, 1000);
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const resendButtons = document.querySelectorAll(".js-resend-button");
    resendButtons.forEach((button) => {
        const rawValue = button.dataset.resendUntil;
        if (!rawValue) {
            return;
        }

        const endDate = new Date(rawValue);
        if (isNaN(endDate.getTime())) {
            return;
        }

        if (endDate.getTime() > Date.now()) {
            startButtonTimer(button, endDate);
        }
    });

    const cooldownNotes = document.querySelectorAll(".js-cooldown-note");
    cooldownNotes.forEach((note) => {
        const rawValue = note.dataset.cooldownUntil;
        if (!rawValue) {
            return;
        }
        ...
        if (endDate.getTime() > Date.now()) {
            const baseText = note.textContent.trim();
            startNoteTimer(note, endDate, baseText);
        }
    });
});
